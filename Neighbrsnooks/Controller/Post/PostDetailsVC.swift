...
//  Neighbrsnooks
//
//  Created by Irshad Malik on 09/05/25.
//
...
@available(iOS 16.0, *)
class PostDetailsVC:BaseViewController,UICollectionViewDelegateFlowLayout,UICollectionViewDelegate,UICollectionViewDataSource,UITextViewDelegate {
...
extension PostDetailsVC: UITableViewDataSource, UITableViewDelegate{
    ...
    func updateTableViewHeight() {
        if let commentCount = CommentPostListData?.postlistdata.count, commentCount > 0 {
            tableviewHeightMess.constant = tableviewPost.contentSize.height
        } else {
            tableviewHeightMess.constant = 0
        }
...
@available(iOS 16.0, *)
extension PostDetailsVC: UIScrollViewDelegate {
