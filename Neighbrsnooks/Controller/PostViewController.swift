...
@available(iOS 16.0, *)
class PostViewController: BaseViewController, MemberCellDelegate,UITextFieldDelegate {
...
extension PostViewController: UITableViewDataSource, UITableViewDelegate, PostTableViewCellDelegate, PostvTableViewCellDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblMonth.font = UIFont(name: "Montserrat-Regular", size: 13)
        cell.lblGeneral.font = UIFont(name: "Montserrat-Medium", size: 14)
        ...
        // let v = "".decodeEmoji
        let text = filteredPostData?.listdata?[indexPath.row].postemojiunicode?.decodeEmoji
        if text == nil {
        ...
        cell.DotCallback = { [weak self] postID in
            ...
            //MARK: - Navigate to MessageViewController
...
//     MARK: - Did select for Post image
    ...
    @objc func collectionViewItemDidSelect(_ notification: Notification) {
        if let indexPath = notification.object as? IndexPath {
            // Handle the selection and navigate to another screen
...
//            UserDefaults.standard.set(self.PostListData?.listdata?.first?.emojilistdata?.first?.postid, forKey: "postid")
//
//
//
//            completionClosure()
