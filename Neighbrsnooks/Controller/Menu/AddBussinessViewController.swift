...
//            self.serviceDropdownData.dataSource = self.serviceName
    ...
    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        ...
        lblCountPdfFile.text = "PDF Files: \(selectedPDFURL != nil ? "1" : "0")"
    ...
    @IBAction func PublishBtn(_ sender: UIButton) {
        ...
        // **Form Validation Pehle karein**
        if tfBussinessName.text?.trimmingCharacters(in: .whitespacesAndNewlines) == "" {
            showAlert(message: "Please Enter Business Name")
            sender.isEnabled = true
            return
        }
        
        if tfTag.text?.trimmingCharacters(in: .whitespacesAndNewlines) == "" {
            showAlert(message: "Please Enter Category")
            sender.isEnabled = true
            return
        }

        // ✅ Ab loader ko start karein
    ...
    func callCreateBussinessWebService(_ completionClosure: @escaping () -> ()) {
        ...
        let dictParams: Dictionary<String, Any> = [
            ...
            "closetime": self.lblState.text ?? "", // change irshad malik
