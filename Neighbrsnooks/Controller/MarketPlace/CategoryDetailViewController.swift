...
import UIKit
@available(iOS 16.0, *)
class CategoryDetailViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource, UITextFieldDelegate {
    ...
    var CatMarketData : CatMarketdetailModel?
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return CatMarketData?.producthomelist?.count ?? 0
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        //    self.lblImgLimit.text = "Max Images: " + (self.EventDetauilData?.eventImgRemainLimit ?? "")
            if let priceString = CatMarketData?.producthomelist?[indexPath.row].salePrice,
            ...
            if CatMarketData?.producthomelist?[indexPath.row].pStatus == 2 /*|| CatMarketData?.producthomelist?[indexPath.row].saleType == "Donate"*/{
                cell.lblSellDonate.isHidden = false
            } else {
                cell.lblSellDonate.isHidden = true
            }
            cell.DayLbl.text = CatMarketData?.producthomelist?[indexPath.row].createdTime
            let url = URL(string: (CatMarketData?.producthomelist?[indexPath.row].pImages ?? ""))
            ...
            cell.DetailCallback = { [self] value in
               ...
               // vc.idD = (MarketWallData?.yourItems?[indexPath.row].id)!
                vc.idD = String(CatMarketData?.producthomelist?[indexPath.row].id ?? 0)
...
//                  let url = "https://dev.neighbrsnook.com/marketplace/api/mpk_product_home?"
//
    ...
    func callMarketcatWebService() {
       ...
       // let dictParams: Dictionary<String, Any> = ["":""]
        ...
        RSNetworkManager.shared.newRequestApi(withServiceName:url,requestMethod:.GET,requestParameters: dictParams, withProgressHUD: true)
          ...
          case .SUCCESS ,.CREATED:
          do {
              let data = try JSONDecoder().decode(CatMarketdetailModel.self, from: result!)
            self.CatMarketData = data
            self.collectionViewMyEvent.reloadData()
