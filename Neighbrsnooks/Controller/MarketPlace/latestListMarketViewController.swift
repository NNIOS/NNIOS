...
import UIKit
@available(iOS 16.0, *)
class latestListMarketViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource {
    ...
    func callMarketLatestWebService() {
                  let url = "https://neighbrsnook.com/admin/api/mpk_product_today_list?"
       ...
       // let dictParams: Dictionary<String, Any> = ["":""]
        ...
        RSNetworkManager.shared.newRequestApi(withServiceName:url,requestMethod:.GET,requestParameters: dictParams, withProgressHUD: true)
          ...
          do {
              let data = try JSONDecoder().decode(LatestListModel.self, from: result!)
            self.LatestListData = data
