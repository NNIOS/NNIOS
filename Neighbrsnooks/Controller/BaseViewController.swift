...
//
    ...
    func formatPrice(_ value: Double) -> String {
            if value < 1000 {
                return "\(Int(value))"
            } else if value < 1_00_000 { // less than 1 lakh
                let formatted = value / 1000
                return String(format: "%.1fK", formatted)
            } else if value < 1_00_00_000 { // less than 1 crore
                let formatted = value / 1_00_000
                return String(format: "%.1fL", formatted)
            } else if value < 10_00_00_000 { // less than 10 crore
                let formatted = value / 1_00_00_000
                return String(format: "%.1fCr", formatted)
            } else if value < 1_000_000_000 { // less than 1 Billion
                let formatted = value / 10_00_00_000
                return String(format: "%.1fCr", formatted)
            } else if value < 10_000_000_000 { // less than 10 Billion
                let formatted = value / 1_000_000_000
                return String(format: "%.1fB", formatted)
            } else {
                let formatted = value / 10_000_000_000
                return String(format: "%.1fB", formatted)
            }
        }
