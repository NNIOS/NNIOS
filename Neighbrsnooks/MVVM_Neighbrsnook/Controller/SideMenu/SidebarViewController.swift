...
//        
        ...
        self.lblUserName.text = self.userProfileData?.name
            let userName = self.userProfileData?.name ?? ""
            let firstLetter = String(userName.prefix(1)).uppercased()
            
            if let imageUrlString = self.userProfileData?.profile_picture,
               let url = URL(string: imageUrlString),
               !imageUrlString.trimmingCharacters(in: .whitespaces).isEmpty {
        ...
        self.lblNeighbourhood.text = self.userProfileData?.neighborhood_name ?? ""
...
//                vc.sourceViewController = "MyProfile"
            ...
            case "Group":
                guard let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "GroupListVC") as? GroupListVC else {
