...
class RegistationAdressProofVC: UIViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate, TOCropViewControllerDelegate{
    ...
    let registationDataVM : RegistationThardV_M? = nil
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        } else if let neighborhood = userProfileData?.neighborhood_name {
            mainText = "Id & Address Proof "
        ...
        print("Profile Data received:", userProfileData ?? "No Data")
        print("Uploaded Docs received:", uploadedDocuments ?? [])
        
        // Gender & DOB

        if let serverGender = userProfileData?.gender {
            // If your options use "Male", "Female", convert to that format
            genderTextField.text = serverGender.capitalized
        }
        dobTextField.text = userProfileData?.date_of_birth ?? ""
...
//            }
//        }
        // Document selection based on uploaded_doc field
        if let docType = userProfileData?.uploaded_doc?.lowercased() {
            if docType.contains("aadhaar") {
                selectDocument(viewAadhaarCard, type: .aadhaar)
            } else if docType.contains("passport") {
                selectDocument(viewPassport, type: .passport)
        ...
        // Load uploaded document images from doc_image field
        if let docImage = userProfileData?.doc_image {
            if let frontURLString = docImage.front, let frontURL = URL(string: frontURLString) {
                loadImage(from: frontURL) { [weak self] image in
                    DispatchQueue.main.async {
                        self?.frontImage = image
                        self?.viewFrontImg.backgroundColor = .white
            ...
            if let backURLString = docImage.back, let backURL = URL(string: backURLString) {
                loadImage(from: backURL) { [weak self] image in
                    DispatchQueue.main.async {
                        self?.backImage = image
                        self?.viewBackImg.backgroundColor = .white
...
//                step2VC.shouldUpdateUIOnAppear = false
    ...
    @IBAction func action_Register(_ sender: Any) {
        ...
        guard
            ...
            genderOptions.contains(where: { $0.caseInsensitiveCompare(genderText) == .orderedSame }),
            genderText.caseInsensitiveCompare(genderOptions[0]) != .orderedSame // Not "Please select"
        else {
    ...
    func RegistationThardSteepApi(_ completionClosure: @escaping () -> ()) {
        // Gender mapping
        var genderValue = ""
        if let genderText = self.genderTextField.text?.lowercased() {
            ...
            case "other": genderValue = "other"
            default: genderValue = ""
            }
        }

        // DOB formatting (yyyy-MM-dd as per API screenshot)
        var formattedDOB = ""
        if let dobString = self.dobTextField.text, !dobString.isEmpty {
            let inputFormatter = DateFormatter()
            inputFormatter.dateFormat = "dd-MM-yyyy"   // user input format
            let outputFormatter = DateFormatter()
            outputFormatter.dateFormat = "yyyy-MM-dd"  // API format
            if let date = inputFormatter.date(from: dobString) {
                formattedDOB = outputFormatter.string(from: date)
            } else {
                formattedDOB = dobString // fallback
            }
        }
        ...
        // ✅ Handle images (new keys as per Postman screenshot)
        var documents: [String: UIImage] = [:]

        switch selectedDocumentType {
        case .aadhaar:
            if let front = frontImage { documents["aadhar_front"] = front }
            if let back = backImage { documents["aadhar_back"] = back }

        case .passport:
            if let front = frontImage { documents["passport_front"] = front }
            if let back = backImage { documents["passport_back"] = back }

        case .voterID:
            if let front = frontImage { documents["voter_id_front"] = front }
            if let back = backImage { documents["voter_id_back"] = back }

        case .drivingLicense:
            if let front = frontImage { documents["driving_license_front"] = front }
            if let back = backImage { documents["driving_license_back"] = back }

        case .rentdocs:
            if let front = frontImage { documents["rent_docs"] = front }

        default:
            print("⚠️ Koi document selected nahi hai")
            return
        }

        // ✅ API call
        RegistationThardV_M.shared.RegistationThard(parameters: dictParams,
                                                   documents: documents) { response in
            DispatchQueue.main.async {
                if let result = response {
                    if result.status == true || result.code == 200 {
                        print("✅ Registration Step 3 Success: \(result.message)")
                        completionClosure()
                    } else {
