...
class CreatePollViewController: UIViewController, UITextViewDelegate {
    ...
    var objCreatePoll: CreatePollResponse?
    ...
    func createpollApi() {
       ...
       let  request = CreatePoll_Request(title: tvQuest.text ?? "", question: tvQuest.text ?? "", options: optionArray, start_date: tfStartDatee.text ?? "", end_date: tfEndDate.text ?? "")
        ...
        print("Create poll param is: \(param)")
        let ViewModel = CreatePoll_VM()
        ViewModel.createPoll(parameter: param, request: request) { response in
            DispatchQueue.main.async {
                if let encryptedData = response {
