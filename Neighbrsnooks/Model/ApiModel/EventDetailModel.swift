...
//    let userattends: Int?
//    let userjoinmemberlist, userunjoinmemberlist: [String]?
...
//    let userunjoinmemberlist: [String]?
//    let userlikes, datetimeandneighbrhood, createby: String?
//    let userpic: String?
//    let userid, futureeventstatus, iseventrunning, totalLike: String?
//    let totalJoin, unjoin, nojoin, eventImgLimit: String?
...
enum UserUnjoinMemberList: Codable {
       case userList([Userjoinmemberlist])
       case stringList([String])
       
       init(from decoder: Decoder) throws {
           let container = try decoder.singleValueContainer()
           if let userList = try? container.decode([Userjoinmemberlist].self) {
               self = .userList(userList)
           } else if let stringList = try? container.decode([String].self) {
               self = .stringList(stringList)
           } else {
               throw DecodingError.typeMismatch(UserUnjoinMemberList.self, DecodingError.Context(codingPath: decoder.codingPath, debugDescription: "Expected value of type [Userjoinmemberlist] or [String]"))
           }
       }
       ...
       func encode(to encoder: Encoder) throws {
           ...
           case .userList(let userList):
               try container.encode(userList)
           case .stringList(let stringList):
               try container.encode(stringList)
           }
       }
   }
