...
extension Date
{
  ...
  static func getChatMessageTime(seconds: Double) -> String {
    ...
    let messageDate = Date.getDateFromUnixTimeStamp(timeStamp: seconds)
    ...
    if timeDifference >= 0 {
      ...
      if Int(hour) >= 1 && Int(hour) < 24 {
        ...
        if messageDate.isYesterday() {
          ...
          return "yesterday at " + messageDate.stringWithFormat(format: "hh:mm a")
        } else {
          
          return messageDate.stringWithFormat(format: "hh:mm a")
        }
        ...
        return messageDate.stringWithFormat(format: "dd-MMM-yyyy")
      }
    } else {
